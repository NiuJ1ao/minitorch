...
def avgpool2d(input: Tensor, kernel: Tuple[int, int]) -> Tensor:
    ...
    batch, channel, height, width = input.shape
    # TODO: Implement for Task 4.3.
    input_tile, height, width = tile(input, kernel)
    output = input_tile.mean(dim=-1)
    return output.view(batch, channel, height, width)
...
def logsoftmax(input: Tensor, dim: int) -> Tensor:
    ...
    return input - input_max - (input - input_max).exp().sum(dim=dim).log()
